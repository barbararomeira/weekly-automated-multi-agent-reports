...
def _system_prompt(methodology: str, schema: dict) -> str:
    return (
        ...
        "data (a fabricated number, a wrong customer/line name, a conclusion the "
        "data does not support). Pipeline halts; cannot be fixed by re-rendering "
        "text.\n"
        "- FIXABLE: a methodology rule violation the agent can correct by "
        "rewriting the offending block (missing unit, wrong unit, forbidden "
        "framing such as period-aggregate language, judgement words like "
        "'disappointing' / 'bad' / 'failure', formatting issues). The "
        "orchestrator will hand each fixable warning back to the Insights agent "
        "for a targeted re-emit.\n\n"
